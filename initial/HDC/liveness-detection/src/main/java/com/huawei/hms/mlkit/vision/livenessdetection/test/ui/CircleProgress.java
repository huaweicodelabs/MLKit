...
/**
 * @Description 圆形进度条，类似 QQ 健康中运动步数的 UI 控件
 *
 * @Author FuLin
 * @Time 2022/1/4 22:16
 */
public class CircleProgress extends View {
    ...
    private int[] mGradientColors = {Color.GREEN, Color.YELLOW, Color.BLUE};
    ...
    private void init(Context context, AttributeSet attrs) {
        mContext = context;
        mDefaultSize = CircleProgressUtil.dipToPx(mContext, CircleProgressConstant.DEFAULT_SIZE);
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mHintSize =
                typedArray.getDimension(
                        R.styleable.CircleProgressBar_hintSize, CircleProgressConstant.DEFAULT_HINT_SIZE);

        mValue = typedArray.getFloat(R.styleable.CircleProgressBar_value, CircleProgressConstant.DEFAULT_VALUE);
        mMaxValue =
                typedArray.getFloat(R.styleable.CircleProgressBar_maxValue, CircleProgressConstant.DEFAULT_MAX_VALUE);
        // 内容数值精度格式
        mPrecision = typedArray.getInt(R.styleable.CircleProgressBar_precision, 0);
        mPrecisionFormat = CircleProgressUtil.getPrecisionFormat(mPrecision);
        mValueColor = typedArray.getColor(R.styleable.CircleProgressBar_valueColor, Color.BLACK);
        mValueSize =
                typedArray.getDimension(
                        R.styleable.CircleProgressBar_valueSize, CircleProgressConstant.DEFAULT_VALUE_SIZE);
        ...
        mUnitSize =
                typedArray.getDimension(
                        R.styleable.CircleProgressBar_unitSize, CircleProgressConstant.DEFAULT_UNIT_SIZE);

        mArcWidth =
                typedArray.getDimension(
                        R.styleable.CircleProgressBar_arcWidth, CircleProgressConstant.DEFAULT_ARC_WIDTH);
        mStartAngle =
                typedArray.getFloat(
                        R.styleable.CircleProgressBar_startAngle, CircleProgressConstant.DEFAULT_START_ANGLE);
        mSweepAngle =
                typedArray.getFloat(
                        R.styleable.CircleProgressBar_sweepAngle, CircleProgressConstant.DEFAULT_SWEEP_ANGLE);

        mBgArcColor = typedArray.getColor(R.styleable.CircleProgressBar_bgArcColor, Color.WHITE);
        mBgArcWidth =
                typedArray.getDimension(
                        R.styleable.CircleProgressBar_bgArcWidth, CircleProgressConstant.DEFAULT_ARC_WIDTH);
    ...
    private void drawArc(Canvas canvas) {
        ...
        canvas.drawArc(mRectF, 10, currentAngle, false, mArcPaint);
        canvas.restore();
    }

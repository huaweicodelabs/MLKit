...
import com.huawei.hms.mlkit.vision.livenessdetection.test.BuildConfig;
...
public class WaveProgress extends View {
    ...
    private void init(Context context, AttributeSet attrs) {
        mDefaultSize = CircleProgressUtil.dipToPx(context, CircleProgressConstant.DEFAULT_SIZE);
        mRectF = new RectF();
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mDarkWaveAnimTime =
                typedArray.getInt(R.styleable.WaveProgress_darkWaveAnimTime, CircleProgressConstant.DEFAULT_ANIM_TIME);
        mLightWaveAnimTime =
                typedArray.getInt(R.styleable.WaveProgress_lightWaveAnimTime, CircleProgressConstant.DEFAULT_ANIM_TIME);
        mMaxValue = typedArray.getFloat(R.styleable.WaveProgress_maxValue, CircleProgressConstant.DEFAULT_MAX_VALUE);
        mValue = typedArray.getFloat(R.styleable.WaveProgress_value, CircleProgressConstant.DEFAULT_VALUE);
        mValueSize =
                typedArray.getDimension(R.styleable.WaveProgress_valueSize, CircleProgressConstant.DEFAULT_VALUE_SIZE);
        ...
        mHintSize =
                typedArray.getDimension(R.styleable.WaveProgress_hintSize, CircleProgressConstant.DEFAULT_HINT_SIZE);

        mCircleWidth =
                typedArray.getDimension(R.styleable.WaveProgress_circleWidth, CircleProgressConstant.DEFAULT_ARC_WIDTH);
        ...
        mWaveHeight =
                typedArray.getDimension(
                        R.styleable.WaveProgress_waveHeight, CircleProgressConstant.DEFAULT_WAVE_HEIGHT);
        mWaveNum = typedArray.getInt(R.styleable.WaveProgress_waveNum, 1);
        mDarkWaveColor =
                typedArray.getColor(
                        R.styleable.WaveProgress_darkWaveColor,
                        getResources().getColor(android.R.color.holo_blue_dark));
        mLightWaveColor =
                typedArray.getColor(
                        R.styleable.WaveProgress_lightWaveColor,
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(
                CircleProgressUtil.measure(widthMeasureSpec, mDefaultSize),
                CircleProgressUtil.measure(heightMeasureSpec, mDefaultSize));
    }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int minSize =
                Math.min(
                        getMeasuredWidth() - getPaddingLeft() - getPaddingRight() - 2 * (int) mCircleWidth,
    ...
    private Point[] getPoint(boolean isR2L, float waveWidth) {
        ...
        for (int i = 0; i < mHalfPointCount; i++) {
            ...
            points[i] =
                    new Point(
                            (isR2L ? 2 : 1) * points[mHalfPointCount].x - points[reverse].x,
                            points[mHalfPointCount].y * 2 - points[reverse].y);
        }
        // 对从右向左的贝塞尔点数组反序，方便后续处理
        return isR2L ? CircleProgressUtil.reverse(points) : points;
    }
    ...
    private void drawWave(Canvas canvas, Paint paint, Point[] points, float waveOffset) {
        ...
        for (int i = 1; i < mAllPointCount; i += 2) {
            mWavePath.quadTo(
                    points[i].x + waveOffset,
                    points[i].y + height,
                    points[i + 1].x + waveOffset,
                    points[i + 1].y + height);
        }
    ...
    private void startLightWaveAnimator() {
        ...
        mLightWaveAnimator.addUpdateListener(
                new ValueAnimator.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(ValueAnimator animation) {
        ...
        mLightWaveAnimator.addListener(
    ...
    private void startDarkWaveAnimator() {
        ...
        mDarkWaveAnimator.addUpdateListener(
                new ValueAnimator.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(ValueAnimator animation) {
